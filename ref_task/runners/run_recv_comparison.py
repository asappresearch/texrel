"""
this file will only handle receiver comparison
...
from ref_task import params_groups, run_receiver
...
def run(args):
    results = []
    for recv_architecture in args.recv_architectures:
        for hg in args.hgs:
            child_params = Params(dict(args.__dict__))
            child_params.ref = f'{args.ref}_{recv_architecture}_{hg}'
            child_params.ds_tasks = hg
            child_seed = args.seed_base + 0
            child_params.seed = child_seed
            del child_params.__dict__['seed_base']

            child_params.multimodal_classifier = recv_architecture
            if recv_architecture == 'MapUtteranceToConvWeights':
                child_params.batch_size //= 2
            elif recv_architecture == 'LearnedCNNMapping':
                child_params.batch_size //= 8
            print('batch_size', child_params.batch_size)

            runner = run_receiver.Runner()
            ...
            res['recv_architecture'] = recv_architecture
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--recv-architectures', type=str,
        default='Concat,Cosine,FeaturePlaneAttention,AllPlaneAttention,'
                'MapUtteranceToConvWeights')
    parser.add_argument(
        '--hgs', type=str,
    ...
    args.recv_architectures = args.recv_architectures.split(',')
